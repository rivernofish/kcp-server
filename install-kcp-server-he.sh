...
program_download_url=https://github.com/rivernofish/kcp-server/tree/master/latest/
x64_file=server_linux_amd64
x86_file=server_linux_386
md5sum_file=md5sum.md
program_init_download_url=https://raw.githubusercontent.com/rivernofish/kcp-server/master/kcp-server.init
str_install_shell=https://raw.githubusercontent.com/rivernofish/kcp-server/master/install-kcp-server-he.sh
    ...
    if [ -s /etc/init.d/kcp-server ] || [ -s ${str_program_dir}/kcp-server ] ; then
        ...
        if [ ! -z ${remote_shell_version} ] || [ ! -z ${remote_init_version} ];then
            ...
            if [[ "${version}" < "${remote_shell_version}" ]];then
                echo "========== Update kcp-Server install-kcp-server-he.sh =========="
                if ! wget --no-check-certificate ${str_install_shell} -O ${install_shell}/install-kcp-server.sh; then
                    echo "Failed to download install-kcp-server-he.sh file!"
                    exit 1
                else
                    echo -e "${COLOR_GREEN}install-kcp-server-he.sh Update successfully !!!${COLOR_END}"
                    update_flag="true"
                fi
            fi
